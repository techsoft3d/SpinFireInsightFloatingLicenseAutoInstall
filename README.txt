...
-------------
  ...
  4. Both license files saved somewhere accessible on this machine:
       - license.al
       - sfpflv2.dat
     These are typically emailed from TechSoft3D support.
     The easiest option is to save them to your Downloads folder before running.

HOW TO RUN
----------
  Option A — Easiest (recommended):
    Double-click "Run-Setup.bat"
    Approve the Administrator (UAC) prompt when it appears.
...
WHAT TO EXPECT
--------------
  The script will walk you through the following automatically:

  1. Checks whether the Floating License Manager is already installed.
     If not, it downloads and silently installs it.

  2. Asks where your license files are located.
     Press Enter to check your Downloads folder automatically.

  3. Validates that the license was issued for this machine by checking
     the hostname and MAC address in sfpflv2.dat.
     You will be warned (but can continue) if there is a mismatch.

  4. Copies the license files to the FLM install directory.

  5. Registers lmgrd as a Windows Service and starts it.

  6. Adds inbound Windows Firewall rules for lmgrd.exe, spinfired.exe,
     and TCP port 27000.
...
-------------------------------------
  After setup completes, configure each SpinFire Insight workstation to
  use the following license server address:

      27000@<this-server-hostname>

  Example:  27000@rh00vmgfps01

  You can find your server hostname in the setup summary or by running:
      hostname
  in a Command Prompt on this machine.
...
---------------
  License server fails to start:
    ...
    - Also check the setup log on your Desktop.

  Hostname or MAC address mismatch warning:
    - The license files were generated for a specific server (hostname + MAC).
    - If the server was renamed or this is a different machine, a new license
      file must be requested from TechSoft3D.
  ...
  Firewall/connectivity issues from client machines:
    ...
    - Confirm no other firewall (e.g., third-party AV) is blocking lmgrd.exe
      or spinfired.exe.
    - Verify clients can reach this server by hostname (DNS resolution).

  Script blocked by execution policy:
    - Use Option C above, or double-click Run-Setup.bat which bypasses this.
...
-------
  ...
  Email: spinfiresupport@techsoft3d.com
  Docs:  https://docs.techsoft3d.com/spinfire/insight/setting_up_spinfire/floating_license_manager/
